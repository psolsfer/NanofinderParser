...
from tests.conftest import MockMapping


def test_vendor_version() -> None:
...
def test_frame_header() -> None:
    fh = FrameHeader(
        Vendor="TestVendor",
        Version="1.0",
        Date="2023/01/01",  # type: ignore[arg-type]
        Time="12:00:00",  # type: ignore[arg-type]
        Information="Test",
...
def test_stage_axes_dimensions() -> None:
...
def test_stage_3d_parameters() -> None:
...
def test_channel() -> None:
    ch = Channel(
        ...
        ChannelAxisArray="0 1 2 3 4",  # type: ignore[arg-type]
        ChannelInfo={
...
def test_data_calibration() -> None:
    dc = DataCalibration(
        Vendor="TestVendor",
        Version="1.0",
        Channels=[
...
def test_scanned_frame_parameters() -> None:
...
def test_mapping(sample_mapping_data: MockMapping) -> None:
