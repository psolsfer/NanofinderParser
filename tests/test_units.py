...
def test_setup_spectroscopy_constants(unit_registry: UnitRegistry) -> None:
...
def test_convert_spectral_units(value: Any, unit_in: Any, unit_out: Any, expected: Any) -> None:
    result = convert_spectral_units(value, unit_in, unit_out)
    assert np.isclose(result, expected, rtol=1e-2)


def test_convert_spectral_units_array() -> None:
...
def test_convert_spectral_units_quantity(unit_registry: UnitRegistry) -> None:
...
def test_convert_spectral_units_invalid_unit() -> None:
    with pytest.raises(ValueError, match=r"Invalid input unit: .*"):  # Invalid input unit:
        convert_spectral_units(500, "invalid_unit", "nm")


def test_convert_spectral_units_same_unit() -> None:
    value = 500
