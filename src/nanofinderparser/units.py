...
def convert_spectral_units(
    ...
    if not isinstance(value, Quantity):
        try:
            value_quantity: Quantity = value * units_dict[unit_in]  # type: ignore[reportAssignmentType]
        except KeyError:
    ...
    if unit_in == "raman_shift":
        # Raman shift to cm-1
        value_quantity = laser_wavelength_quantity.to(registry.cm**-1) - value_quantity  # type: ignore[reportAssignmentType]

    if unit_out == "raman_shift":
        converted_value: Quantity = laser_wavelength_quantity.to(
            registry.cm**-1
        ) - value_quantity.to(registry.cm**-1)  # type: ignore[reportAssignmentType]
    else:
